<x-app-layout>
    <div class="space-y-6" x-data="{
        ...
        <div class="flex items-center justify-between border-b border-gray-100 pb-5">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-gray-900">Tambah Berita Baru</h1>
                <p class="text-sm text-gray-500 mt-1">Buat artikel berita baru untuk dipublikasikan ke portal sekolah.</p>
            </div>
            <div>
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white rounded-xl border border-gray-100 p-6 shadow-sm space-y-5">
                    <div class="form-control w-full">
                        ...
                        <input type="text" name="title" value="{{ old('title') }}" placeholder="Ketik judul berita di sini..." class="input input-bordered w-full rounded-lg text-sm border-gray-200 focus:ring-primary focus:border-primary @error('title') input-error @enderror" required />
                        @error('title')
                    ...
                    <div class="form-control w-full">
                        ...
                        <input type="text" name="slug" value="{{ old('slug') }}" placeholder="url-berita-otomatis-jika-kosong" class="input input-bordered w-full rounded-lg text-sm border-gray-200 focus:ring-primary focus:border-primary" />
                        @error('slug')
                            <span class="text-xs text-rose-600 mt-1">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-control w-full">
                        ...
                        <textarea name="excerpt" rows="3" placeholder="Ringkasan berita singkat..." class="textarea textarea-bordered w-full rounded-lg text-sm border-gray-200 focus:ring-primary focus:border-primary">{{ old('excerpt') }}</textarea>
                        @error('excerpt')
                            <span class="text-xs text-rose-600 mt-1">{{ $message }}</span>
                        @enderror
                    </div>
                        ...
                        @forelse($categories as $category)
                            <label class="flex items-center gap-3 cursor-pointer py-0.5">
                                <input type="checkbox" name="categories[]" value="{{ $category->id }}" 
                                    ...
                                    {{ is_array(old('categories')) && in_array((string)$category->id, old('categories')) ? 'checked' : '' }} />
