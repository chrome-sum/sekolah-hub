<x-app-layout>
    <div class="space-y-6" x-data="{
        featuredMediaId: '{{ old('featured_media_id', $page->featured_media_id) }}',
        featuredMediaUrl: '{{ old('featured_media_id', $page->featured_media_id) ? $page->featured_image_url : '' }}',
        selectMedia(id, url) {
        ...
        <div class="flex items-center justify-between border-b border-gray-100 pb-5">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-gray-900">Edit Halaman</h1>
                <p class="text-sm text-gray-500 mt-1">Perbarui informasi halaman statis portal sekolah.</p>
            </div>
            <div>
                <a href="{{ route('admin.pages.index') }}" class="btn btn-ghost btn-sm text-gray-500 hover:text-gray-700 rounded-lg flex items-center gap-1.5">
        ...
        <form action="{{ route('admin.pages.update', $page->id) }}" method="POST" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white rounded-xl border border-gray-100 p-6 shadow-sm space-y-5">
                    <div class="form-control w-full">
                        <label class="label-text text-gray-700 font-semibold mb-1.5 text-[13px]">Judul Halaman</label>
                        <input type="text" name="title" value="{{ old('title', $page->title) }}" placeholder="Ketik judul halaman di sini..." class="input input-bordered w-full rounded-lg text-sm border-gray-200 focus:ring-primary focus:border-primary @error('title') input-error @enderror" required />
                        @error('title')
                    ...
                    <div class="form-control w-full">
                        ...
                        <input type="text" name="slug" value="{{ old('slug', $page->slug) }}" placeholder="url-halaman-otomatis-jika-kosong" class="input input-bordered w-full rounded-lg text-sm border-gray-200 focus:ring-primary focus:border-primary" />
                        @error('slug')
                            <span class="text-xs text-rose-600 mt-1">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-control w-full">
                        <label class="label-text text-gray-700 font-semibold mb-1.5 text-[13px]">Isi Halaman</label>
                        <div class="prose max-w-none border border-gray-200 rounded-lg p-1 bg-gray-50/10">
                            <x-rich-text::trix id="page-content" name="content" :value="old('content', $page->content?->toTrixHtml())" />
                        </div>
                        @error('content')
                            <span class="text-xs text-rose-600 mt-1">{{ $message }}</span>
                        @enderror
                ...
                <div class="bg-white rounded-xl border border-gray-100 p-6 shadow-sm space-y-5">
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-0.5">Optimalkan tampilan halaman Anda di hasil pencarian Google.</p>
                    </div>
                    <div class="border-t border-gray-50 pt-4 space-y-4">
                        <div class="form-control w-full">
                            ...
                            <input type="text" name="seo_title" value="{{ old('seo_title', $page->seo_title) }}" placeholder="Judul khusus untuk hasil pencarian Google" class="input input-bordered w-full rounded-lg text-sm border-gray-200 focus:ring-primary focus:border-primary" />
                        </div>

                        <div class="form-control w-full">
                            <label class="label-text text-gray-700 font-semibold mb-1.5 text-[13px]">SEO Description</label>
                            <textarea name="seo_description" rows="3" placeholder="Deskripsi khusus untuk Google snippet" class="textarea textarea-bordered w-full rounded-lg text-sm border-gray-200 focus:ring-primary focus:border-primary">{{ old('seo_description', $page->seo_description) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-xl border border-gray-100 p-6 shadow-sm space-y-5">
                    ...
                    <div class="form-control w-full">
                        <label class="label-text text-gray-700 font-semibold mb-1.5 text-[13px]">Parent Halaman</label>
                        <select name="parent_id" class="select select-bordered w-full rounded-lg text-sm border-gray-200 focus:ring-primary focus:border-primary">
