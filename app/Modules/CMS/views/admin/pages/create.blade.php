<x-app-layout>
    <div class="space-y-6" x-data="{
        ...
        <div class="flex items-center justify-between border-b border-gray-100 pb-5">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-gray-900">Tambah Halaman Baru</h1>
                <p class="text-sm text-gray-500 mt-1">Buat halaman statis baru untuk portal sekolah.</p>
            </div>
            <div>
        ...
        <form action="{{ route('admin.pages.store') }}" method="POST" class="grid grid-cols-1 lg:grid-cols-3 gap-6" x-data="{ loading: false }" @submit="loading = true">
            @csrf
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white rounded-xl border border-gray-100 p-6 shadow-sm space-y-5">
                    <div class="form-control w-full">
                        ...
                        <input type="text" name="title" value="{{ old('title') }}" placeholder="Ketik judul halaman di sini..." class="input input-bordered w-full rounded-lg text-sm border-gray-200 focus:ring-primary focus:border-primary @error('title') input-error @enderror" required />
                        @error('title')
                    ...
                    <div class="form-control w-full">
                        <label class="label-text text-gray-700 font-semibold mb-1.5 text-[13px]">Slug (Opsional)</label>
                        <input type="text" name="slug" value="{{ old('slug') }}" placeholder="url-halaman-otomatis-jika-kosong" class="input input-bordered w-full rounded-lg text-sm border-gray-200 focus:ring-primary focus:border-primary" />
                        @error('slug')
                            <span class="text-xs text-rose-600 mt-1">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-control w-full">
                        ...
                        <div class="prose max-w-none border border-gray-200 rounded-lg p-1 bg-gray-50/10">
                            <x-rich-text::trix id="page-content" name="content" :value="old('content')" />
                        </div>
                        @error('content')
                            <span class="text-xs text-rose-600 mt-1">{{ $message }}</span>
                        @enderror
                ...
                <div class="bg-white rounded-xl border border-gray-100 p-6 shadow-sm space-y-5">
                    ...
                    <div class="border-t border-gray-50 pt-4 space-y-4">
                        <div class="form-control w-full">
                            ...
                            <input type="text" name="seo_title" value="{{ old('seo_title') }}" placeholder="Judul khusus untuk hasil pencarian Google" class="input input-bordered w-full rounded-lg text-sm border-gray-200 focus:ring-primary focus:border-primary" />
                        </div>

                        <div class="form-control w-full">
                            <label class="label-text text-gray-700 font-semibold mb-1.5 text-[13px]">SEO Description</label>
                            <textarea name="seo_description" rows="3" placeholder="Deskripsi khusus untuk Google snippet" class="textarea textarea-bordered w-full rounded-lg text-sm border-gray-200 focus:ring-primary focus:border-primary">{{ old('seo_description') }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
