    ...
    <nav class="flex mb-5 text-[11px] font-semibold text-gray-400 uppercase tracking-wider select-none" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-1.5">
            <li class="inline-flex items-center">
                <a href="{{ route('dashboard') }}" class="inline-flex items-center text-gray-400 hover:text-primary transition">
            ...
            @foreach($segments as $index => $segment)
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        @else
                            <a href="{{ url($currentUrl) }}" class="ml-1 md:ml-1.5 text-gray-400 hover:text-primary transition">{{ $title }}</a>
                        @endif
                    </div>
                </li>
            @endforeach
